...
# fields to `config`. So pos-migration/<net>/genesis.json must stay identical to
# the live <net>/metadata/genesis.json apart from those added keys. This catches
# the draft silently drifting away from the chain it is supposed to migrate.
#
# Usage: scripts/check_pos_migration.sh [joc|joct|all]   (default: all)
set -euo pipefail
...
print(f"== {net} ==")
live = json.load(open(f"{root}/{net}/metadata/genesis.json"))
draft = json.load(open(f"{root}/pos-migration/{net}/genesis.json"))
...
case "$TARGET" in
  all) rc=0; run joc || rc=1; echo; run joct || rc=1; exit $rc ;;
  joc|joct) run "$TARGET" ;;
